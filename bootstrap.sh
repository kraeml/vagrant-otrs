...
install_apache () {
	sudo apt-get install -y apache2 \
	 build-essential libapache-dbi-perl \
	 libapache2-mod-perl2 libapache2-mod-perl2 \
	 libarchive-zip-perl libarchive-zip-perl \
	 libcrypt-eksblowfish-perl libdbd-mysql-perl \
	 libdbd-mysql-perl libdbd-mysql-perl libdbi-perl \
	 libdigest-md5-file-perl libdigest-perl libgd-graph-perl \
	 libgd-text-perl libio-socket-ssl-perl libjson-xs-perl \
	 libjson-xs-perl libnet-dns-perl libnet-dns-perl libnet-ldap-perl \
	 libpdf-api2-perl libsoap-lite-perl libtemplate-perl libtemplate-perl \
	 libtext-csv-xs-perl libtext-csv-xs-perl libtimedate-perl libxml-parser-perl \
	 libyaml-libyaml-perl libyaml-libyaml-perl perl > /dev/null
}

install_otrs_dependencies () {
	sudo apt-get install -y libencode-hanextra-perl libgd-gd2-perl libdigest-md5-perl
}
